...
KeyValueErrorCode ReadQuotedString(const char*& str, kvString_t& inset)
{
	...
	*/
	...
	if (!*str)
	{
		...
		return KeyValueErrorCode::KV_INCOMPLETE_STRING;
	}
...
KeyValueErrorCode KeyValue::Parse(const char*& str, const bool isRoot)
{
	...
	for (; *str;)
	{
		...
		case BLOCK_END:
			...
			if (isRoot)
				return KeyValueErrorCode::KV_UNEXPECTED_END_OF_BLOCK;
		...
		case 0:
			...
			return KeyValueErrorCode::KV_INCOMPLETE_BLOCK;

		case BLOCK_BEGIN:
			return KeyValueErrorCode::KV_UNEXPECTED_START_OF_BLOCK;
