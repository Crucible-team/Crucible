...
#include "../Crucible/wiProfiler.h"
#include "../Crucible/wiBacklog.h"
#include "../Crucible/wiPrimitive.h"
#include "../Crucible/wiRenderPath3D.h"
...
void style_classic_steam(void);
void style_rounded_visual_studio(void);
void style_visual_studio(void);
void style_future_dark(void);
...
void Example_ImGui::Initialize()
{
	...
	//style_dark_ruda();
	//style_classic_steam();
	//style_rounded_visual_studio();
	//style_visual_studio();
	style_future_dark();

	add_my_font("./Content/Roboto-Medium.ttf");
...
void Example_ImGuiRenderer::Load()
{
	...
	viewport2.camera = &viewport2Cam;
	viewport3.camera = &viewport3Cam;
	viewport4.camera = &viewport4Cam;

	//viewport3.SetWireRender(true);
	//wi::renderer::SetWireRender(true);
	viewport1.setFXAAEnabled(true);
	viewport2.setFXAAEnabled(true);
	wi::eventhandler::SetVSync(true);
	wi::renderer::ClearWorld(wi::scene::GetScene());
	wi::scene::GetScene().weather = WeatherComponent();
	viewport1.setAO(wi::RenderPath3D::AO_SSAO);
	viewport2.setAO(wi::RenderPath3D::AO_SSAO);
...
void Example_ImGuiRenderer::Update(float dt)
{
	...
	if (ImGui::Begin("DockSpaceWicked", &dockingopen, window_flags))
	{
		...
		if (ImGui::DockBuilderGetNode(ImGui::GetID("MyDockspace")) == NULL || redock_windows_on_start)
		{
			...
			ImGuiID dock_id_rightTop = ImGui::DockBuilderSplitNode(dock_id_right, ImGuiDir_Up, 0.5f, NULL, &dock_id_right);
			ImGuiID dock_id_middleright = ImGui::DockBuilderSplitNode(dock_main_id, ImGuiDir_Right, 0.5f, NULL, &dock_main_id);
			ImGuiID dock_id_middlebottomleft = ImGui::DockBuilderSplitNode(dock_main_id, ImGuiDir_Down, 0.5f, NULL, &dock_main_id);
			ImGuiID dock_id_middlebottomright = ImGui::DockBuilderSplitNode(dock_id_middleright , ImGuiDir_Down, 0.5f, NULL, &dock_id_middleright);
			...
			ImGui::DockBuilderDockWindow("Scene", dock_id_rightTop);
			ImGui::DockBuilderDockWindow("Options", dock_id_right);
			ImGui::DockBuilderDockWindow("ViewPort 1", dock_main_id);
	...
	ImGui::Begin("Tools");
	{
		ImGui::BeginColumns("##quickbar", 1, ImGuiOldColumnFlags_NoBorder | ImGuiOldColumnFlags_NoResize);
	...
	ImGui::Begin("Scene");
	{
		...
		if (ImGui::CollapsingHeader(ICON_MD_DATA_OBJECT "  Entities", ImGuiTreeNodeFlags_DefaultOpen)) //ImGuiTreeNodeFlags_None
		{
			ImGui::Indent(4);
			ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(0.0f, 3.0f));
			auto size = scene.names.GetCount();
			...
			{
				PushToEntityTree(scene.softbodies.GetEntity(i));
			}
			...
			/*for (size_t i = 0; i < size; ++i)
			{
				...
				bool opened = ImGui::TreeNodeEx((void*)(uint64_t)e,flags,s.c_str());
	...
	ImGui::Begin("Options");
	{
		...
		if (ImGui::CollapsingHeader(ICON_MD_SETTINGS "  Settings", ImGuiTreeNodeFlags_None)) //ImGuiTreeNodeFlags_DefaultOpen
		{
			...
			static bool bEyeAdaption = viewport1.getEyeAdaptionEnabled();
			if (ImGui::Checkbox("Eye Adaption", &bEyeAdaption))
			{
				viewport1.setEyeAdaptionEnabled(bEyeAdaption);
			}
			...
			{
				viewport1.setFXAAEnabled(bFXAAEnabled);
			}
		...
		if (ImGui::CollapsingHeader(ICON_MD_SCREENSHOT_MONITOR "  Camera Screenshot", ImGuiTreeNodeFlags_None)) //ImGuiTreeNodeFlags_DefaultOpen
		{
			...
			Texture* lpTexture = (Texture * ) viewport1.lastPostprocessRT;
			if (lpTexture)
			{
	...
	ImGui::Begin("ViewPort 1",&bAlwaysOpen,ImGuiWindowFlags_NoScrollbar);
	{
		ImVec2 viewport1size = ImGui::GetContentRegionAvail();

		viewport1.width = viewport1size.x;
		viewport1.height = viewport1size.y;
		wi::graphics::Texture SceneView;
		static MaterialComponent::TextureMap tmScene;

		SceneView = viewport1.rtMain;

		tmScene.resource.SetTexture(SceneView);
		tmScene.name = "rtMain";
		Texture* lpTexture = (Texture*)tmScene.GetGPUResource();
	...
	ImGui::Begin("ViewPort 2",&bAlwaysOpen,ImGuiWindowFlags_NoScrollbar);
	{
		ImVec2 viewport1size = ImGui::GetContentRegionAvail();

		viewport2.width = viewport1size.x;
		viewport2.height = viewport1size.y;
		wi::graphics::Texture SceneView;
		static MaterialComponent::TextureMap tmScene;

		SceneView = viewport2.rtMain;
	...
	ImGui::Begin("ViewPort 3",&bAlwaysOpen,ImGuiWindowFlags_NoScrollbar);
	{
		ImVec2 viewport1size = ImGui::GetContentRegionAvail();

		viewport3.width = viewport1size.x;
		viewport3.height = viewport1size.y;
		wi::graphics::Texture SceneView;
		static MaterialComponent::TextureMap tmScene;
	...
	ImGui::Begin("ViewPort 4",&bAlwaysOpen,ImGuiWindowFlags_NoScrollbar);
	{
		ImVec2 viewport1size = ImGui::GetContentRegionAvail();

		viewport4.width = viewport1size.x;
		viewport4.height = viewport1size.y;
		wi::graphics::Texture SceneView;
		static MaterialComponent::TextureMap tmScene;

		SceneView = viewport4.rtMain;
	...
	ImGui::Begin(ICON_MD_BUG_REPORT " Debugger", &bAlwaysOpen, ImGuiWindowFlags_None); //ImGuiWindowFlags_NoScrollbar
	{
		...
		debug_textures.push_back(viewport1.rtMain); debug_texture_name.push_back("rtMain");
		//debug_textures.push_back(active_render->rtPrimitiveID); debug_texture_name.push_back("rtPrimitiveID"); //UINT shader option
		debug_textures.push_back(viewport1.rtBloom); debug_texture_name.push_back("rtBloom");
		debug_textures.push_back(viewport1.rtBloom_tmp); debug_texture_name.push_back("rtBloom_tmp");
		...
		debug_textures.push_back(viewport1.rtSceneCopy); debug_texture_name.push_back("rtSceneCopy");
		debug_textures.push_back(viewport1.rtSceneCopy_tmp); debug_texture_name.push_back("rtSceneCopy_tmp");
		debug_textures.push_back(viewport1.rtReflection); debug_texture_name.push_back("rtReflection");
		...
		debug_textures.push_back(viewport1.rtPostprocess); debug_texture_name.push_back("rtPostprocess");
		debug_textures.push_back(viewport1.rtShadingRate); debug_texture_name.push_back("rtShadingRate");
		debug_textures.push_back(viewport1.rtLinearDepth); debug_texture_name.push_back("rtLinearDepth");
		...
		debug_textures.push_back(viewport1.rtSSR); debug_texture_name.push_back("rtSSR");
		debug_textures.push_back(viewport1.rtAO); debug_texture_name.push_back("rtAO");
		debug_textures.push_back(viewport1.rtShadow); debug_texture_name.push_back("rtShadow");
	...
	//PE: Debugger display some of the textures used.
	ImGui::Begin("Assets", &bAlwaysOpen); //ImGuiWindowFlags_NoScrollbar
	{
		int columns = 7;
		ImGui::BeginColumns("##assets", columns, ImGuiOldColumnFlags_NoBorder | ImGuiOldColumnFlags_NoResize);
	...
	ImGui::Begin(ICON_MD_TEXT_SNIPPET " Backlog", &bAlwaysOpen, ImGuiWindowFlags_NoScrollbar);
	{
...
void Example_ImGuiRenderer::DisplayPerformanceData(bool* p_open)
{
	...
	{
		ImGui::Text("Crucible Engine (%s) - FPS: %.1f", wi::version::GetVersionString(),ImGui::GetIO().Framerate);
	}
	ImGui::End();
}
...
void Example_ImGuiRenderer::PushToEntityTree(Entity entity)
{
	...
	bool opened = ImGui::TreeNodeEx((void*)(uint64_t)entity,flags,s.c_str());
	if (ImGui::IsItemClicked())
	{
